...
function Field({ label, error, children }: { label: string; error?: string; children: React.ReactNode }) {
  ...
  return (
    <View style={fieldStyles.container}>
      <Text style={[fieldStyles.label, { color: theme.textTertiary }]}>{label}</Text>
      {children}
      {error && <Text style={[fieldStyles.error, { color: theme.destructive }]}>{error}</Text>}
    </View>
  );
}
...
const fieldStyles = StyleSheet.create({
  container: { gap: 6 },
  label: { fontFamily: Typography.fontFamily.primarySemiBold, fontSize: Typography.sizes.caption, textTransform: 'uppercase', letterSpacing: 0.6 },
...
const styles = StyleSheet.create({
  ...
  segBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 4, paddingVertical: 10, borderRadius: BorderRadius.sm },
  segText: { fontFamily: Typography.fontFamily.primarySemiBold, fontSize: Typography.sizes.caption },
  input: { borderWidth: 1, borderRadius: BorderRadius.md, paddingHorizontal: Spacing.md, height: 48, fontFamily: Typography.fontFamily.primary, fontSize: Typography.sizes.body },
