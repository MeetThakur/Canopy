const token = 'eyJhbGciOiJIUzI1NiJ9..KIHyF8_0E4i9mhxvVNkUwN93cqQQW42SyZSgWunqeTw';

async function test() {
  const q = `
    query {
      __schema {
        queryType {
          fields {
            name
            args {
              name
            }
          }
        }
      }
    }
  `;
  const res = await fetch('https://api.hardcover.app/v1/graphql', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
    body: JSON.stringify({ query: q })
  });
  const data = await res.json();
  const search = data.data.__schema.queryType.fields.find(f => f.name === 'search');
  const books_trending = data.data.__schema.queryType.fields.find(f => f.name === 'books_trending');
  console.log('search args:', JSON.stringify(search));
  console.log('trending args:', JSON.stringify(books_trending));
}
test();
