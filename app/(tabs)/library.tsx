  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.background }]}>
      <View style={styles.headerRow}>
        ...
        <TouchableOpacity onPress={toggleView} style={[styles.iconBtn, { backgroundColor: theme.surface2 }]} accessibilityLabel="Toggle view mode">
          {viewMode === 'list' ? <LayoutGrid size={20} color={theme.textPrimary} /> : <List size={20} color={theme.textPrimary} />}
        </TouchableOpacity>
      </View>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.pillRow}>
        {STATUS_FILTERS.map((f) => {
          ...
          return (
            <TouchableOpacity key={f.value} onPress={() => setActiveStatus(f.value)}
              style={[styles.pill, { backgroundColor: active ? theme.textPrimary : theme.surface2 }]}
              accessibilityLabel={`Filter by ${f.label}`}>
              <Text style={[styles.pillText, { color: active ? theme.background : theme.textSecondary }]}>{f.label}</Text>
            </TouchableOpacity>
          );
        })}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  gridCard: { height: 200, borderRadius: BorderRadius.md, overflow: 'hidden', backgroundColor: '#2E2C2A' },
  gridCover: { width: '100%', height: '100%' },
  gridOverlay: { ...StyleSheet.absoluteFillObject, justifyContent: 'flex-end', padding: Spacing.sm, gap: Spacing.xs, backgroundColor: 'rgba(0,0,0,0.5)' },
  gridTitle: { fontFamily: Typography.fontFamily.primarySemiBold, fontSize: Typography.sizes.bodySmall, color: '#FFF' },
  fab: {
