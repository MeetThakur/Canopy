...
function ResultRow({ item }: { item: MediaSearchResult }) {
  const { isDark } = useTheme();
  const theme = isDark ? Colors.dark : Colors.light;
  return (
    <TouchableOpacity style={[styles.resultRow, { borderBottomColor: theme.border }]} accessibilityLabel={item.title}>
      <Image source={{ uri: item.coverUrl || undefined }} style={styles.resultCover} contentFit="cover" transition={200} />
      <View style={styles.resultInfo}>
        <Text style={[styles.resultTitle, { color: theme.textPrimary }]} numberOfLines={1}>{item.title}</Text>
        <Text style={[styles.resultSub, { color: theme.textSecondary }]} numberOfLines={1}>{item.subtitle}</Text>
...
export default function SearchScreen() {
  ...
  const libraryItems = useLibraryStore((s) => s.getItems());
  ...
  const webResults = results.filter((r) =>
    !libraryItems.find((l) => l.sourceId === r.sourceId)
  );
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.background }]}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipsRow}>
        {CATEGORY_CHIPS.map((chip) => {
          ...
          return (
            <TouchableOpacity key={chip.value} onPress={() => setCategory(chip.value)}
      ...
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        {!loading && query.length === 0 && recentSearches.length > 0 && (
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              <Text style={[styles.sectionTitle, { color: theme.textSecondary }]}>Recent Searches</Text>
              <TouchableOpacity onPress={clearRecentSearches}><Text style={[styles.clearText, { color: theme.destructive }]}>Clear</Text></TouchableOpacity>
            </View>
            {recentSearches.map((r) => (
              <TouchableOpacity key={r} style={[styles.recentRow, { borderBottomColor: theme.border }]}
                onPress={() => { setQuery(r); search(); }} accessibilityLabel={`Recent search: ${r}`}>
        ...
        {!loading && libraryMatches.length > 0 && (
          <View style={styles.section}>
            <Text style={[styles.sectionTitle, { color: theme.textSecondary }]}>In Your Library</Text>
            {libraryMatches.map((item) => (
              <ResultRow key={item.id} item={{ id: item.id, sourceId: item.sourceId, type: item.type, title: item.title, subtitle: item.subtitle, coverUrl: item.coverUrl, year: item.year }} />
            ))}
          </View>
        )}
        ...
        {!loading && webResults.length > 0 && (
          <View style={styles.section}>
            <Text style={[styles.sectionTitle, { color: theme.textSecondary }]}>From the Web</Text>
            {webResults.map((item) => <ResultRow key={item.id} item={item} />)}
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  searchBar: {
    ...
    paddingHorizontal: Spacing.md, height: 48, borderRadius: BorderRadius.lg, borderWidth: 1,
  },
  ...
  recentRow: { paddingVertical: Spacing.md, borderBottomWidth: StyleSheet.hairlineWidth },
  recentText: { fontFamily: Typography.fontFamily.primary, fontSize: Typography.sizes.body },
  resultRow: { flexDirection: 'row', paddingVertical: Spacing.md, borderBottomWidth: StyleSheet.hairlineWidth, gap: Spacing.md },
  resultCover: { width: 48, height: 68, borderRadius: BorderRadius.sm, backgroundColor: '#2E2C2A' },
  ...
  noResults: { textAlign: 'center', marginTop: Spacing.xl, fontFamily: Typography.fontFamily.primary, fontSize: Typography.sizes.body },
});
